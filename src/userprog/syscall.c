...
/* Poj2 system call */
static tid_t handler_pthread_create(stub_fun sfun, pthread_fun tfun, void *arg, struct process *pcb);
...
static void syscall_handler(struct intr_frame *f UNUSED) {
  ...
  case SYS_PT_CREATE:
    ...
    if (beneath) {
      f->eax = beneath ? handler_pthread_create((stub_fun)args[1], (pthread_fun)args[2], (void *)args[3], pcb)
                       : TID_ERROR;
    }
    break;
  ...
  case SYS_SEMA_INIT:
    beneath = check_boundary(args + 1);
    if (beneath) {
      f->eax = check_not_null(args[1]) ? handler_sema_init((sema_t *)args[1], args[2], pcb) : false;
    }
    break;
  case SYS_SEMA_DOWN:
    beneath = check_boundary(args + 1);
