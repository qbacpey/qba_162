...
2. 废弃`in_handler`，转而使用`pending_thread`和`exiting`控制进程退出时尚位于内核中线程的行为：
   1. 考虑到系统在同一时间仅可能处理一个硬件中断，因此硬件中断无需维护`pending_thread`;
   2. 系统在进入系统调用处理程序的时候需要`++pending_thread`，离开时需要`--pending_thread`;
   3. 下面是一些特殊情况：
      - `thread_create`: 进入之前需要再次`++pending_thread`，应对此函数执行过程中发生进程退出的情形；
